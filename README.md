...
## Remote環境での再利用運用（複数ユーザー向け）

本リポジトリは、remote環境上で「パイプライン本体」と「各解析プロジェクト」を分離して再利用できます。
既存の構築済み環境がある場合、毎回 `setup_env.sh` をフル実行する必要はありません。

### 推奨ディレクトリ構成（例）

```text
/home/user/Desktop/data/RNAseq_test/
├── RNAseq_pipeline_takubo/        # パイプライン本体（このrepo）
├── reference/                     # 共有リファレンス（再利用）
│   ├── STAR2.7.3/
│   │   ├── hg38_sjdb149_RefSeq/
│   │   └── mm10_sjdb149_RefSeq_ver241129/
│   └── Genome/
│       ├── hg38/
│       │   └── hg38.ncbiRefSeq.gtf
│       └── mm10/
...
### 新規ユーザーが新しい解析プロジェクトを開始する手順

```bash
# 1) パイプラインと既存環境を利用
cd /home/user/Desktop/data/RNAseq_test/RNAseq_pipeline_takubo
conda activate rnaseq_takubo

# 2) 新規プロジェクトを初期化
bash init_project.sh /home/user/Desktop/data/RNAseq_test/projects/<PROJECT_NAME> hg38

# 3) config.sh で共有referenceを参照（重要）
# STAR_INDEX_HG38=/home/user/Desktop/data/RNAseq_test/reference/STAR2.7.3/hg38_sjdb149_RefSeq
# GTF_HG38=/home/user/Desktop/data/RNAseq_test/reference/Genome/hg38/hg38.ncbiRefSeq.gtf

# 4) サンプル情報とFASTQを準備
vim /home/user/Desktop/data/RNAseq_test/projects/<PROJECT_NAME>/samples.tsv

# 5) 解析実行
bash run_pipeline.sh --config /home/user/Desktop/data/RNAseq_test/projects/<PROJECT_NAME>/config.sh
```

### 共有運用ルール（推奨）

- `reference/` は共通資産として扱い、原則プロジェクトごとに複製しない
- 各プロジェクトは `init_project.sh` で作成し、`--config` 指定で実行する
- `rnaseq_takubo` が存在する場合は再作成せず `conda activate rnaseq_takubo` を使用する
- パッケージ更新が必要な場合のみ `bash setup_env.sh --update` を実行する
- プロジェクトごとの `samples.tsv` / `plot_targets.tsv` / `config.sh` を分離して管理する

### coding agent向けメモ

本環境では以下を前提にすると既存資産を最大限再利用できます。

- pipeline root: `/home/user/Desktop/data/RNAseq_test/RNAseq_pipeline_takubo`
- shared reference root: `/home/user/Desktop/data/RNAseq_test/reference`
- conda env: `rnaseq_takubo`
- 新規解析は `bash init_project.sh <project_dir> <genome>` で開始
- 実行は `bash run_pipeline.sh --config <project_dir>/config.sh`
